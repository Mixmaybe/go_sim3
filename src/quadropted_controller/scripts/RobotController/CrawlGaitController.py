#!/usr/bin/env python3
# Authors: mike4192 https://github.com/mike4192/spotMicro, lnotspotl ,abutalipovvv
import numpy as np
import rclpy
...
class CrawlSwingController:
    def __init__(self, stance_ticks, swing_ticks, time_step, phase_length, z_leg_lift, default_stance, body_shift_y):
    ...
    def swing_height(self, swing_phase):
        ...
        return self.z_leg_lift * np.sin(np.pi * swing_phase)
    ...
    def next_foot_location(self, swing_prop, leg_index, state, command, shifted_left):
        ...
        time_left = max(self.time_step * self.swing_ticks * (1.0 - swing_prop), self.time_step)
...
class CrawlStanceController:
    ...
    def position_delta(self, leg_index, state, command, first_cycle, move_sideways, move_left):
        ...
        side_vel = 0.0
        if move_sideways:
            side_vel = -(self.body_shift_y * shift_factor) / (float(self.time_step) * self.stance_ticks) if move_left \
                        else (self.body_shift_y * shift_factor) / (float(self.time_step) * self.stance_ticks)
